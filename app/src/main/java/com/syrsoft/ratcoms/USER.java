...
public class USER {
    ...
    public static List<JobTitle> PurchaseOrdersJobTitles, BonusAuthsJobTitles, ExitAuthsJobTitles, JobTitles, RequestCustodyAuthsJobTitles, ResignationsAuthsJobtitles, VacationsAuthJobtitles, BacksAuthJobtitles, AdvancePaymentsAuthJobtitles, VacationSalaryAuthJobtitles, PenaltyAuthJobTitles, ChamberAuthJobTitles;
    public static List<USER> PurchaseAuthUsers, BonusAuthUsers, EMPS, ExitAuthUsers, CustodyAuthUsers, ResignationsAuthUsers, VacationsAuthUsers, BacksAuthUsers, AdvancePaymentaAuthUsers, VacationSalaryAuthUsers, PenaltyAuthUsers, ChamberAuthUsers;
    ...
    public static List<PUNISHMENT_CLASS> PenaltiesList;

    public static List<CHAMBER_COMMERCE_CLASS> chamberCommerceList;
    public static List<LOCAL_PURCHASE_ORDER> purchaseList;
    public static List<List<USER>> PurchaseOrdersAuthUsers, BonusOrdersAuthUsers, ExitOrdersAuthUsers, VacationOrdersAuthUsers, ResignationOrdersAuthUsers, VacationSalaryOrdersAuthUsers, CustodyOrdersAuthUsers, BacksOrdersAuthUsers, AdvancesOrdersAuthUsers, penaltyOrdersAuthUsers,chamberCommerceAuthUsers;
    ...
    static String getRequestDiscountForApproval = "https://ratco-solutions.com/RatcoManagementSystem/NewOptions/getRequestDiscountForApproval.php";
            ...
            public void onResponse(String response) {
                ...
                if (!response.equals("0")) {
                    ...
                    if (lis.size() > 0) {
                        ...
                        Log.d("punishmentApprove","getEmps2 "+MyApp.EMPS.size());
                    }
...
//                }
    ...
    void getRequestPenaltyAuthEmps(VollyCallback callback) {
        PenaltyAuthJobTitles = new ArrayList<>();
        PenaltyAuthUsers = new ArrayList<>();
        MyApp.PunishmentAuthJobtitles.clear();
        for (int i = 0; i < Types.size(); i++) {
            if (Types.get(i).HROrderName.equals("RequestDiscount")) {
                if (Types.get(i).Auth1 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth1));
                }
                if (Types.get(i).Auth2 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth2));
                }
                if (Types.get(i).Auth3 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth3));
                }
                if (Types.get(i).Auth4 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth4));
                }
                if (Types.get(i).Auth5 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth5));
                }
                if (Types.get(i).Auth6 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth6));
                }
                if (Types.get(i).Auth7 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth7));
                }
                if (Types.get(i).Auth8 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth8));
                }
                if (Types.get(i).Auth9 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth9));
                }
                if (Types.get(i).Auth10 != 0) {
                    PenaltyAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth10));
                }
            }
        }
        if (PenaltyAuthJobTitles.size() > 0) {
            MyApp.PunishmentAuthJobtitles = PenaltyAuthJobTitles;
            Log.d("PenaltiesResponse", PenaltyAuthJobTitles.size() + " jobTitles");
            for (int p = 0; p < PenaltyAuthJobTitles.size(); p++) {
                if (PenaltyAuthJobTitles.get(p).JobTitle.equals("Direct Manager")) {
                    USER direct = USER.searchUserByJobNumber(MyApp.EMPS, MyApp.MyUser.DirectManager);
                    if (direct != null) {
                        if (direct.VacationStatus == 0) {
                            PenaltyAuthUsers.add(direct);
                        } else {
                            direct = USER.searchUserByJobNumber(MyApp.EMPS, direct.VacationAlternative);
                            if (direct != null) {
                                PenaltyAuthUsers.add(direct);
                ...
                if (PenaltyAuthJobTitles.get(p).JobTitle.equals("Department Manager")) {
                    USER departmentM = USER.searchUserByJobNumber(MyApp.EMPS, MyApp.MyUser.DepartmentManager);
                    if (departmentM != null) {
                        if (departmentM.VacationStatus == 0) {
                            PenaltyAuthUsers.add(departmentM);
                        } else {
                            departmentM = USER.searchUserByJobNumber(MyApp.EMPS, departmentM.VacationAlternative);
                            if (departmentM != null) {
                                PenaltyAuthUsers.add(departmentM);
                    ...
                    USER user = USER.searchUserByJobtitle(MyApp.EMPS, PenaltyAuthJobTitles.get(p).JobTitle);
                    if (user != null) {
                        if (user.VacationStatus == 0) {
                            PenaltyAuthUsers.add(user);
                        } else {
                            user = USER.searchUserByJobNumber(MyApp.EMPS, user.VacationAlternative);
                            if (user != null) {
                                PenaltyAuthUsers.add(user);
            ...
            MyApp.PunishmentAuthUsers = PenaltyAuthUsers;
            Log.d("PenaltiesResponse", PenaltyAuthUsers.size() + " authUsers");
        }
        callback.onSuccess("done");
    }

    static void getPenaltyRequests(VollyCallback callback) {
        PenaltiesList = new ArrayList<>();
        StringRequest request = new StringRequest(Request.Method.POST, getRequestDiscountForApproval, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Log.d("penaltyResponse", response);
                if (!response.equals("0")) {
                    ...
                    Log.d("penaltyResponse", PenaltiesList.size() + " ");
                    Collections.reverse(PenaltiesList);
                    callback.onSuccess("done");
                } else {
                    callback.onSuccess("done");
                }
            ...
            @Override
            public void onErrorResponse(VolleyError error) {
                Log.d("penaltyResponse", error.toString());
            }
        }) {
            @Override
            protected Map<String, String> getParams() throws AuthFailureError {
    ...
    static void setTheListsOfAuthUsersOFPenalty(VollyCallback callback) {
        Log.d("PenaltiesResponse", PenaltiesList.size() + " PenaltiesList");
        penaltyOrdersAuthUsers = new ArrayList<>();
        for (int i = 0; i < PenaltiesList.size(); i++) {
            List<USER> ll = new ArrayList<>();
            for (int j = 0; j < MyApp.PunishmentAuthJobtitles.size(); j++) {
                if (MyApp.PunishmentAuthJobtitles.get(j).JobTitle.equals("Direct Manager")) {
                    USER direct = USER.searchUserByJobNumber(MyApp.EMPS, USER.searchUserByJobNumber(MyApp.EMPS, PenaltiesList.get(i).JobNumber).DirectManager);
                    if (direct != null) {
                        if (direct.VacationStatus == 0) {
                            ll.add(direct);
                        } else if (direct.VacationStatus == 1) {
                ...
                else if (MyApp.PunishmentAuthJobtitles.get(j).JobTitle.equals("Department Manager")) {
                    USER departmentM = USER.searchUserByJobNumber(MyApp.EMPS, USER.searchUserByJobNumber(MyApp.EMPS, PenaltiesList.get(i).JobNumber).DepartmentManager);
                    if (departmentM != null) {
                        if (departmentM.VacationStatus == 0) {
                            ll.add(departmentM);
                        } else if (departmentM.VacationStatus == 1) {
                    ...
                    USER user = USER.searchUserByJobtitle(MyApp.EMPS, MyApp.PunishmentAuthJobtitles.get(j).JobTitle);
                    if (user != null) {
                        if (user.VacationStatus == 0) {
                            ll.add(user);
                        } else if (user.VacationStatus == 1) {
            ...
            penaltyOrdersAuthUsers.add(ll);
        }
        Log.d("PenaltiesResponse", " usersLists" + penaltyOrdersAuthUsers.size());
        int[] rowsToRemove = new int[PenaltiesList.size()];
        for (int i = 0; i < penaltyOrdersAuthUsers.size(); i++) {
            boolean s = false;
            for (USER u : penaltyOrdersAuthUsers.get(i)) {
                if (u.JobNumber == MyApp.db.getUser().JobNumber) {
                    s = true;
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < rowsToRemove.length; i++) {
                if (rowsToRemove[i] == 1) {
                    PenaltiesList.remove(i - removed);
                    penaltyOrdersAuthUsers.remove(i - removed);
                    removed++;
                }
            }
        } catch (Exception e) {
        }
        MyApp.PenaltyOrdersAuthUsers = penaltyOrdersAuthUsers;
        MyApp.HRCounter = MyApp.HRCounter + PenaltiesList.size();
        MyApp.MYApprovalsCounter = MyApp.MYApprovalsCounter + PenaltiesList.size();
    ...
    void getPenaltyData(Context c, VollyCallback callback) {
        getRequestPenaltyAuthEmps(new VollyCallback() {
            @Override
            public void onSuccess(String res) {
                getPenaltyRequests(new VollyCallback() {
                    @Override
                    public void onSuccess(String res) {
                        setTheListsOfAuthUsersOFPenalty(new VollyCallback() {
                            @Override
                            public void onSuccess(String res) {
                                callback.onSuccess("done");
                            }
    ...
    // get ChamberCommerce orders

    void getRequestChamberCommerceAuthEmps(VollyCallback callback) {
        ChamberAuthJobTitles = new ArrayList<>();
        ChamberAuthUsers = new ArrayList<>();
        MyApp.ChamberAuthJobtitles.clear();
        for (int i = 0; i < Types.size(); i++) {
            if (Types.get(i).HROrderName.equals("RequestChamberCommerce")) {
                if (Types.get(i).Auth1 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth1));
                }
                if (Types.get(i).Auth2 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth2));
                }
                if (Types.get(i).Auth3 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth3));
                }
                if (Types.get(i).Auth4 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth4));
                }
                if (Types.get(i).Auth5 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth5));
                }
                if (Types.get(i).Auth6 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth6));
                }
                if (Types.get(i).Auth7 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth7));
                }
                if (Types.get(i).Auth8 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth8));
                }
                if (Types.get(i).Auth9 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth9));
                }
                if (Types.get(i).Auth10 != 0) {
                    ChamberAuthJobTitles.add(com.syrsoft.ratcoms.HRActivities.JobTitle.searchJobTitleById(JobTitles, Types.get(i).Auth10));
                }
            }
        }
        if (ChamberAuthJobTitles.size() > 0) {
            MyApp.ChamberAuthJobtitles = ChamberAuthJobTitles;
            Log.d("ChamberResponse", ChamberAuthJobTitles.size() + " jobTitles");
            for (int p = 0; p < ChamberAuthJobTitles.size(); p++) {
                if (ChamberAuthJobTitles.get(p).JobTitle.equals("Direct Manager")) {
                    USER direct = USER.searchUserByJobNumber(MyApp.EMPS, MyApp.MyUser.DirectManager);
                    if (direct != null) {
                        if (direct.VacationStatus == 0) {
                            ChamberAuthUsers.add(direct);
                        } else {
                            direct = USER.searchUserByJobNumber(MyApp.EMPS, direct.VacationAlternative);
                            if (direct != null) {
                                ChamberAuthUsers.add(direct);
                ...
                if (ChamberAuthJobTitles.get(p).JobTitle.equals("Department Manager")) {
                    USER departmentM = USER.searchUserByJobNumber(MyApp.EMPS, MyApp.MyUser.DepartmentManager);
                    if (departmentM != null) {
                        if (departmentM.VacationStatus == 0) {
                            ChamberAuthUsers.add(departmentM);
                        } else {
                            departmentM = USER.searchUserByJobNumber(MyApp.EMPS, departmentM.VacationAlternative);
                            if (departmentM != null) {
                                ChamberAuthUsers.add(departmentM);
                    ...
                    USER user = USER.searchUserByJobtitle(MyApp.EMPS, ChamberAuthJobTitles.get(p).JobTitle);
                    if (user != null) {
                        if (user.VacationStatus == 0) {
                            ChamberAuthUsers.add(user);
                        } else {
                            user = USER.searchUserByJobNumber(MyApp.EMPS, user.VacationAlternative);
                            if (user != null) {
                                ChamberAuthUsers.add(user);
            ...
            MyApp.ChamberAuthUsers = ChamberAuthUsers;
            Log.d("ChamberResponse", ChamberAuthUsers.size() + " authUsers");
        }
        callback.onSuccess("done");
    }

    static void getRequestChamberCommerce(VollyCallback callback) {
        chamberCommerceList = new ArrayList<>();
        StringRequest request = new StringRequest(Request.Method.POST, getRequestChamberCommerceForApproval, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                if (!response.equals("0")) {
                    ...
                    Log.d("ChamberCommerceResponse", chamberCommerceList.size() + " ");
    ...
    static void setTheListsOfAuthUsersOFChamberCommerce(VollyCallback callback) {
        Log.d("chamberCommerceResponse", chamberCommerceList.size() + " chamberCommerceList");

        chamberCommerceAuthUsers = new ArrayList<>();
        for (int i = 0; i < chamberCommerceList.size(); i++) {
            List<USER> ll = new ArrayList<>();
            for (int j = 0; j < MyApp.ChamberAuthJobtitles.size(); j++) {
                if (MyApp.ChamberAuthJobtitles.get(j).JobTitle.equals("Direct Manager")) {
                    USER direct = USER.searchUserByJobNumber(MyApp.EMPS, USER.searchUserByJobNumber(MyApp.EMPS, chamberCommerceList.get(i).JobNumber).DirectManager);
                    if (direct != null) {
                        if (direct.VacationStatus == 0) {
                            ll.add(direct);
                        } else if (direct.VacationStatus == 1) {
                ...
                } else if (MyApp.ChamberAuthJobtitles.get(j).JobTitle.equals("Department Manager")) {
                    USER departmentM = USER.searchUserByJobNumber(MyApp.EMPS, USER.searchUserByJobNumber(MyApp.EMPS, chamberCommerceList.get(i).JobNumber).DepartmentManager);
                    if (departmentM != null) {
                        if (departmentM.VacationStatus == 0) {
                            ll.add(departmentM);
                        } else if (departmentM.VacationStatus == 1) {
                    ...
                    USER user = USER.searchUserByJobtitle(MyApp.EMPS, MyApp.ChamberAuthJobtitles.get(j).JobTitle);
                    if (user != null) {
                        if (user.VacationStatus == 0) {
                            ll.add(user);
                        } else if (user.VacationStatus == 1) {
            ...
            chamberCommerceAuthUsers.add(ll);
        }
        Log.d("chamberCommerceResponse", " usersLists" + chamberCommerceAuthUsers.size());
        int[] rowsToRemove = new int[chamberCommerceList.size()];
        for (int i = 0; i < chamberCommerceAuthUsers.size(); i++) {
            boolean s = false;
            for (USER u : chamberCommerceAuthUsers.get(i)) {
                if (u.JobNumber == MyApp.db.getUser().JobNumber) {
                    s = true;
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < rowsToRemove.length; i++) {
                if (rowsToRemove[i] == 1) {
                    chamberCommerceList.remove(i - removed);
                    chamberCommerceAuthUsers.remove(i - removed);
                    removed++;
                }
            }
        } catch (Exception e) {
        }
        MyApp.ChamberOrdersAuthUsers = chamberCommerceAuthUsers;
        MyApp.HRCounter = MyApp.HRCounter + PenaltiesList.size();
        MyApp.MYApprovalsCounter = MyApp.MYApprovalsCounter + chamberCommerceList.size();
    ...
    void getChamberCommerceData(Context c, VollyCallback callback) {
        getRequestChamberCommerceAuthEmps(new VollyCallback() {
            @Override
            public void onSuccess(String res) {
                getRequestChamberCommerce(new VollyCallback() {
                    @Override
                    public void onSuccess(String res) {
                        setTheListsOfAuthUsersOFChamberCommerce(new VollyCallback() {
                            @Override
                            public void onSuccess(String res) {
                                callback.onSuccess("done");
                            }
                                                                                    ...
                                                                                    public void onSuccess(String res) {
                                                                                        ...
                                                                                        getBonusData(c, new VollyCallback() {
                                                                                            ...
                                                                                            public void onSuccess(String s) {
                                                                                                ...
                                                                                                getPenaltyData(c, new VollyCallback() {
                                                                                                    @Override
                                                                                                    public void onSuccess(String s) {
                                                                                                        Log.d("newProsedur", "get Penalty Done");
                                                                                                        getChamberCommerceData(c, new VollyCallback() {
                                                                                                            @Override
                                                                                                            public void onSuccess(String s) {
                                                                                                                Log.d("newProsedur", "get ChamberCommerce Done");
