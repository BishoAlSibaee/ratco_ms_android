...
public class MyApproval_BONUS_REQUEST_ADAPTER extends RecyclerView.Adapter<MyApproval_BONUS_REQUEST_ADAPTER.HOLDER> {

    List<BONUS> list ;
    boolean[] status ;

    public MyApproval_BONUS_REQUEST_ADAPTER(List<BONUS> list) {
        this.list = list ;
    }
    ...
    @NonNull
    @Override
    public MyApproval_BONUS_REQUEST_ADAPTER.HOLDER onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull MyApproval_BONUS_REQUEST_ADAPTER.HOLDER holder, @SuppressLint("RecyclerView") int position) {
        ...
        if (MyApprovals.Status == 0) {
            if (MyApp.BonusOrdersAuthUsers.get(position).size()>0) {
                for (int i = 0; i< MyApp.BonusOrdersAuthUsers.get(position).size(); i++) {
                    ...
                    holder.apps.get(i).setText(MyApp.BonusOrdersAuthUsers.get(position).get(i).FirstName+" "+MyApp.BonusOrdersAuthUsers.get(position).get(i).LastName);
            ...
            if (MyApprovals.BonusOrdersAuthUsers.get(position).size()>0) {
                for (int i = 0; i< MyApprovals.BonusOrdersAuthUsers.get(position).size(); i++) {
                    holder.appsCaption.get(i).setText(MyApp.BonusAuthsJobTitles.get(i).JobTitle);
                    holder.apps.get(i).setText(MyApprovals.BonusOrdersAuthUsers.get(position).get(i).FirstName+" "+MyApprovals.BonusOrdersAuthUsers.get(position).get(i).LastName);
                    holder.appsres.get(i).setText(list.get(position).Accs.get(i).getAuthResult());
                    holder.appsdate.get(i).setText("");
                    holder.appNotes.get(i).setText(list.get(position).Accs.get(i).Note);
                    if (list.get(position).Accs.get(i).getAcc() == 1) {
        ...
        holder.approve.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (holder.myNotes.getText() == null || holder.myNotes.getText().toString().isEmpty() ) {
                    new MESSAGE_DIALOG(holder.itemView.getContext(),"Please Enter Note","");
                    return;
                }
                boolean result = true ;
                for (Acceptance a : list.get(position).Accs) {
                    if (a.getAcc() == 2 ) {
                        result = false ;
                    }
                }
