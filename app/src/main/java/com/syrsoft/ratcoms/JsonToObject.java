...
public class JsonToObject {
    ...
    public static List<Object> translate(String res, Class o, Context co) {
        ...
        try {
            ...
            if (name.equals("com.syrsoft.ratcoms.USER")) {
            ...
            else if (name.equals("com.syrsoft.ratcoms.HRActivities.PUNISHMENT_CLASS")) {
                for (int i = 0; i < arr.length(); i++) {
                    JSONObject row = arr.getJSONObject(i);
                    int id = row.getInt("id");
                    int empId = row.getInt("EmpID");
                    ...
                    int EmpJn = row.getInt("EmployeeJobNumber");
                    ...
                    int status = row.getInt("Status");
                    PUNISHMENT_CLASS r = new PUNISHMENT_CLASS(id, empId, jnum, fname, lname, Dmanager, Dmn, jtitle, reason, amount, sdate, EmpName, EmpJn, status, auths);
                    List.add(r);
                    Log.d("type", "");
                }

            }
            else if (name.equals("com.syrsoft.ratcoms.HRActivities.CHAMBER_COMMERCE_CLASS")) {
                for (int i = 0; i < arr.length(); i++) {
                    JSONObject row = arr.getJSONObject(i);
                    int id = row.getInt("id");
                    int empId = row.getInt("EmpID");
                    ...
                    String ContractForm = row.getString("ContractForm");
                    String sdate = row.getString("SendDate");
                    ...
                    int status = row.getInt("Status");
                    CHAMBER_COMMERCE_CLASS C = new CHAMBER_COMMERCE_CLASS(id, empId, jnum, fname, lname, Dmanager, Dmn, jtitle, Title, ContractForm, sdate, status, auths);
                    List.add(C);
                    Log.d("type", "");
                }
            } else {
                ...
                Log.d("LoginErrorrr", name + "CHAMBER_COMMERCE");
            }
